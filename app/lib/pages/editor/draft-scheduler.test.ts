...
import { createDraftScheduler, type SchedulerClock, type TimerHandle } from './draft-scheduler'
...
describe('createDraftScheduler', () => {
  it('debounces rapid edits into one write carrying the latest payload', () => {
    ...
    clock.run()
    expect(writes).toEqual([{ path: '/rec/a.mp3', cuts: [{ start: 1, end: 4 }] }])
  })
  ...
  it('writes to the path captured at SCHEDULE time, never a later one', () => {
    ...
    clock.run()
    expect(writes).toEqual([{ path: '/rec/a.mp3', cuts: [{ start: 5, end: 9 }] }])
  })
