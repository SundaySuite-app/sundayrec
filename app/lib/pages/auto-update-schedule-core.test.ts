...
describe("planAutoUpdateSchedule", () => {
  it("arms nothing that is already armed", () => {
    expect(planAutoUpdateSchedule(true, true)).toEqual({
      start: false,
      stop: false,
    });
  });
  ...
  it("arms when enabled and no timer is running", () => {
    expect(planAutoUpdateSchedule(false, true)).toEqual({
      start: true,
      stop: false,
    });
  });
  ...
  it("stops the running timer when the setting goes off", () => {
    expect(planAutoUpdateSchedule(true, false)).toEqual({
      start: false,
      stop: true,
    });
  });
  ...
  it("has nothing to stop when disabled and idle", () => {
    expect(planAutoUpdateSchedule(false, false)).toEqual({
      start: false,
      stop: false,
    });
  });
