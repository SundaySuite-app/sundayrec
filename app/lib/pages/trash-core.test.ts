...
describe("withoutTrashed", () => {
  ...
  it("keeps rows that have no path — a failed run is still yours to tidy", () => {
    ...
    expect(
      withoutTrashed(
        rows,
        trashedPaths([entry({ originalPath: "/rec/a.mp3" })]),
      ),
...
describe("toTrashRows", () => {
  ...
  it("counts whole days and the days left before the sweep", () => {
    const [row] = toTrashRows(
      [entry({ deletedAt: NOW - 3 * DAY - 1000 })],
      NOW,
    );
    ...
    const [row] = toTrashRows(
      [
        entry({
          related: [
            {
              originalPath: "/rec/a.meta.json",
              trashedPath: "/t/1-a.meta.json",
            },
            {
              originalPath: "/rec/a.peaks.json",
              trashedPath: "/t/1-a.peaks.json",
            },
          ],
        }),
      ],
