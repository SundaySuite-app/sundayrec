...
const tn = (
  ...
  tf(
    `${key}.${new Intl.PluralRules("nb-NO").select(count)}`,
    { n: count, ...params },
    fallback,
  );
...
describe("formatNextDate", () => {
  ...
  it("is the long date for a slot start", () => {
    expect(formatNextDate(stateWith({ next: buildNext(AT) }), ctx())).toBe(
      `DATE@${AT_MS}`,
    );
  });
  ...
  it("appends the special name when there is one", () => {
    const next = buildNext(AT, [special("2026-06-07", "Julaften", "11:00")]);
    expect(formatNextDate(stateWith({ next }), ctx())).toBe(
      `DATE@${AT_MS} · Julaften`,
    );
  });
});
...
describe("formatSidebarStatus", () => {
  it("a running take wins over everything", () => {
    ...
    const out = formatSidebarStatus(state, ctx(), {
      connected: false,
      name: "Qu-5",
    });
...
describe("missed + preflight formatting", () => {
  ...
  it("degrades to the bare label when the timestamp is unusable", () => {
    expect(formatMissed({ at: "garbage", label: "Gudstjeneste" }, ctx())).toBe(
      "Gudstjeneste",
    );
  });
