...
async function makeHarness(
  opts: { autoStart?: number | "manual" } = {},
): Promise<Harness> {
  vi.resetModules();
  const calls: string[] = [];
  let listener: ((p: unknown) => void) | null = null;
  let pending: { resolve: (n: number) => void; reject: () => void } | null =
    null;
...
describe("recording guard", () => {
  it("does not ask for a metering session while a recording owns the device", async () => {
    const h = await makeHarness();
    (
      globalThis as unknown as { window: { __isRecording: boolean } }
