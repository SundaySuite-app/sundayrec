...
describe('buildConfirm', () => {
  ...
  it('uses caller labels verbatim', () => {
    ...
    expect(spec.buttons.map(b => b.label)).toEqual(['Fortsett opptak', 'Ja, stopp'])
  })
...
describe('buildPrompt', () => {
  it('seeds the input from defaultValue and defaults to single-line', () => {
    ...
    expect(spec.input).toEqual({ value: 'Gudstjeneste', placeholder: '', multiline: false })
  })

  it('honours multiline', () => {
    expect(buildPrompt({ title: 'x', multiline: true }).input?.multiline).toBe(true)
  })
...
describe('buildSelect', () => {
  ...
  it('carries the options through untouched', () => {
    expect(buildSelect({ title: 'Velg skjerm', options }).options).toEqual(options)
  })
