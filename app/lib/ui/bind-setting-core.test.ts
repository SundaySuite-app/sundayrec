...
describe("planCommit", () => {
  ...
  it("honours immediate and explicit overrides", () => {
    ...
    expect(
      planCommit("text", { immediate: true, debounceMs: 900 }).debounceMs,
    ).toBe(0);
  });
...
describe("coerceValue", () => {
  ...
  it("passes text and select values through untouched", () => {
    expect(coerceValue("text", { value: " Alta Frikirke " })).toBe(
      " Alta Frikirke ",
    );
...
describe("validateNumber", () => {
  ...
  it("clamps instead when the rule asks for it", () => {
    ...
    expect(
      validateNumber(99999, { min: 500, max: 50000, clamp: true }),
    ).toEqual({
      ok: true,
      value: 50000,
      bound: 50000,
  ...
  it("accepts in-range values and enforces integers when asked", () => {
    expect(validateNumber(90, { min: 1, max: 3650 })).toEqual({
      ok: true,
      value: 90,
    });
...
describe("guardReasonFor", () => {
  ...
  it("guards while a recording is running", () => {
    expect(guardReasonFor({ isRecording: true, nextAtMs: null }, now)).toBe(
      "recording",
    );
  });
  ...
  it("guards inside the wake-lead window before the next start", () => {
    expect(
      guardReasonFor({ isRecording: false, nextAtMs: now + 4 * 60_000 }, now),
    ).toBe("imminent");
    // Exactly at the lead boundary still counts — the machine is already waking.
    expect(
      guardReasonFor({ isRecording: false, nextAtMs: now + 10 * 60_000 }, now),
    ).toBe("imminent");
  });

  it("stays quiet when the next recording is far away, absent or already past", () => {
    expect(
      guardReasonFor({ isRecording: false, nextAtMs: now + 11 * 60_000 }, now),
    ).toBeNull();
    expect(
      guardReasonFor({ isRecording: false, nextAtMs: null }, now),
    ).toBeNull();
    expect(
      guardReasonFor({ isRecording: false, nextAtMs: now - 60_000 }, now),
    ).toBeNull();
  });
  ...
  it("takes a custom lead window", () => {
    expect(
      guardReasonFor(
        { isRecording: false, nextAtMs: now + 20 * 60_000 },
        now,
        30,
      ),
    ).toBe("imminent");
