...
import {
  ...
  RE_ASK,
  promptCopyFor,
} from "./telemetry-consent-copy-core";
...
describe("consent prompt copy", () => {
  ...
  it("says it is asking again, names what was added, and disclaims the old answer", () => {
    ...
    expect(d, "must acknowledge the earlier answer").toMatch(
      /svart på dette før/,
    );
    ...
    expect(d, "must not name the retired companion addition").not.toMatch(
      /kapittelmerker/,
    );
    ...
    expect(d, "must say nothing moves before they answer").toMatch(
      /ingenting sendes/,
    );
  });
  ...
  it("carries the same words as no.json, key for key", () => {
    ...
    for (const copy of [FIRST_ASK, RE_ASK]) {
      expect(ob[copy.titleKey.replace("onboarding.", "")]).toBe(
        copy.titleFallback,
      );
      expect(ob[copy.descKey.replace("onboarding.", "")]).toBe(
        copy.descFallback,
      );
    }
  });
  ...
  it("leans on neither answer", () => {
    ...
    for (const copy of [FIRST_ASK, RE_ASK]) {
      expect(copy.descFallback.toLowerCase()).not.toMatch(
        /anbefal|vi håper|vennligst|du bør/,
      );
    }
  });
});
