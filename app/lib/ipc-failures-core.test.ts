...
describe('the IPC failure ring', () => {
  it('remembers failures newest-last and hands them back newest-first', () => {
    ...
    expect(recentFailures(s).map(f => f.cmd)).toEqual(['trash_list', 'recordings_list'])
    expect(recentFailures(s)[0]).toEqual({ cmd: 'trash_list', error: 'bang', at: 2000 })
  })
  ...
  it('is bounded, so a 4-per-second poll cannot grow it without limit', () => {
    ...
    const s = createIpcFailureState()
    for (let i = 0; i < RING_MAX * 3; i++) recordFailure(s, 'recording_status', `e${i}`, i)
  ...
  it('summarises count, distinct commands and the newest timestamp', () => {
    const s = createIpcFailureState()
    expect(failureSummary(s)).toEqual({ count: 0, commands: [], newestAt: null })
