...
| ⚠️ **Automatic deletion of old recordings** (NOT a removal — a SEAM) | the switch «Slett gamle opptak» + the day field are on Avansert, and the library foot says «Slettes automatisk etter N dager». **Nothing deletes.** `recordings_prune` is the only implementation and has no caller (found V1/PR3). Do NOT file the missing deletion as a backend bug — and note the second half: the code hard-deletes where the text promises the Papirkurv, so the fix is an owner decision before it is wiring. |
