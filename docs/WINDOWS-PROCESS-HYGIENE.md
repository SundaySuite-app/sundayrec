...
| **Omstart under opptak**        | Oppdateringens «Start på nytt og installer» går gjennom samme venting: et pågående opptak stoppes pent og fila skrives FERDIG før prosessen byttes ut (`relaunch_plan` → `arm_wait_then` → `relaunch_now`). Omstarten kan ikke angres etterpå — `prevent_exit` er en no-op for `RESTART_EXIT_CODE` — så ventingen skjer først.                        | `update/mod.rs` (`relaunch`)              |
