fn main() {
    ...
    if std::env::var_os("CARGO_FEATURE_UPDATER").is_some() {
        ...
        // Write-if-changed, NOT unconditionally: `tauri_build::build()` emits
        // `rerun-if-changed` for `capabilities/`, so an unconditional write
        // bumped this file's mtime on every build-script run and made the
        // fingerprint permanently stale — every single cargo invocation
        // (check/clippy/test/build, local and CI) recompiled and relinked the
        // whole app crate for nothing (~10 s locally, more on CI runners).
        // Skipping the write when the bytes are already right breaks that
        // self-dirtying loop; a missing or stale file is still (re)written,
        // so feature toggles and corrupted checkouts behave exactly as before.
        if std::fs::read(cap).ok().as_deref() != Some(desired.as_bytes()) {
            std::fs::write(cap, desired).expect("writing generated updater capability");
        }
    } else {
