...
const HTML_PATH = path.join(__dirname, '..', 'legacy', 'renderer', 'index.html')
...
const PROSE = /[A-ZÆØÅa-zæøåÄÖÜäöüéÉèÈ]*[a-zæøåäöüéè][A-ZÆØÅa-zæøåÄÖÜäöüéÉèÈ]{2,}|[A-ZÆØÅa-zæøåÄÖÜäöüéÉèÈ]{2,}[a-zæøåäöüéè]/
...
function selfTest() {
  ...
  if (count !== 3) {
    console.error('SELVTEST FEILET: telle-logikken fant', count, 'noder i fixturen (fasit 3):', nodes)
    console.error('Skrallen kan ikke stoles på — fiks countHardcoded før du rører baselinen.')
    process.exit(2)
  }
}
...
if (args.includes('--write-baseline')) {
  if (count > BASELINE && !args.includes('--force-up')) {
    console.error(`Nekter å HEVE baselinen (${BASELINE} → ${count}) uten --force-up — skrallen går én vei.`)
    process.exit(1)
  }
...
if (count > BASELINE) {
  console.error(`✕ i18n-skralle: ${count} udekkede prosa-tekstnoder i index.html — baselinen er ${BASELINE} (+${count - BASELINE}).`)
  console.error('  Ny UI-tekst skal ha data-i18n-nøkkel (×7 locales). Kjør med --list for å se nodene.')
  process.exit(1)
}
if (count < BASELINE) {
  console.log(`✓ i18n-skralle: ${count}/${BASELINE} — ${BASELINE - count} under baselinen. Stram til: node scripts/check-i18n-hardcoded.mjs --write-baseline`)
} else {
  console.log(`✓ i18n-skralle: ${count} udekkede prosa-tekstnoder (== baseline)`)
}
