...
export function createEtaEstimator(): EtaEstimator {
  ...
  function advance(nowMs: number): EtaReading {
    ...
    const warm =
      moved >= MIN_SAMPLES && nowMs - firstMs >= MIN_SPAN_MS && rateInit && rate > 0
    if (!warm || stalled) {
    ...
    if (displayed === null) {
      ...
      else if (raw > coasted * GROW_TOLERANCE) displayed = coasted + (raw - coasted) * GROW_EASE
  ...
  return {
    push(fraction: number, nowMs: number): EtaReading {
      if (!Number.isFinite(fraction) || !Number.isFinite(nowMs)) return advance(lastMs)
...
export function etaBucket(ms: number | null): EtaBucket {
  ...
  if (minutes < 10) return { kind: 'minutes', value: Math.max(1, Math.round(minutes)) }
  if (minutes < 60) {
