...
describe('buildHealthFindings', () => {
  ...
  it('words a restricted microphone differently — the user cannot grant it themselves', () => {
    const denied = buildHealthFindings({ permissions: { microphone: 'denied' }, t })[0]
    const restricted = buildHealthFindings({ permissions: { microphone: 'restricted' }, t })[0]
  ...
  it('only mentions the camera when video recording is on', () => {
    expect(
      buildHealthFindings({ permissions: { camera: 'denied' }, videoEnabled: false, t }),
    ).toEqual([])
    expect(
      buildHealthFindings({ permissions: { camera: 'denied' }, videoEnabled: true, t }),
    ).toHaveLength(1)
  })

  it('reports a missing ffmpeg sidecar', () => {
    const found = buildHealthFindings({ ffmpeg: { available: false, path: 'ffmpeg' }, t })
