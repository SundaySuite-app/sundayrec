...
describe('errorCode', () => {
  it('extracts the code from the {code, message} rejection shape', () => {
    ...
    expect(
      errorCode({ code: 'validation', message: 'validation: no_save_folder: nothing configured' }),
    ).toBe('no_save_folder')
  })
  ...
  it('strips every AppError category prefix', () => {
    ...
    expect(errorCode('internal: feature_disabled: streaming requires …')).toBe('feature_disabled')
  })
  ...
  it('matches the exact smtp-host wire message (the general-page seam)', () => {
    ...
    expect(errorCode('validation: no_config_smtp_host: smtp host missing')).toBe(
      'no_config_smtp_host',
    )
    ...
    expect(errorCode('validation: no_config: save folder not set')).toBe('no_config')
  })

  it('handles bare codes and already-extracted error strings', () => {
    expect(errorCode('cancelled')).toBe('cancelled')
    expect(errorCode({ error: 'feature_disabled: not in this build' })).toBe('feature_disabled')
