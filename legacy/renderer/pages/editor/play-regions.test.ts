...
describe('resolvePosition', () => {
  ...
  it('keeps the recording end in main and the next instant in outro', () => {
    ...
    expect(resolvePosition(600.5, full)).toEqual({ region: 'outro', offset: 0.5 })
  ...
  it('never routes to a zero-length region', () => {
    ...
    expect(resolvePosition(-1, { duration: 600, introDur: 0, outroDur: 3 }).region).toBe('main')
    expect(resolvePosition(700, { duration: 600, introDur: 5, outroDur: 0 }).region).toBe('main')
  })
...
describe('regionPosToTimeline', () => {
  it('round-trips every region', () => {
    for (const sec of [-5, -2.5, -0.001, 0, 1, 599.9, 600, 600.5, 603]) {
      ...
      expect(regionPosToTimeline(pos.region, pos.offset, full)).toBeCloseTo(sec, 6)
    }
  })
...
describe('nextRegion', () => {
  ...
  it('stops at the end of the recording when there is no outro', () => {
    ...
    expect(nextRegion('main', { duration: 600, introDur: 5, outroDur: 0 })).toBeNull()
  })
...
describe('routePlayback', () => {
  it('streams the original for formats the webview decodes', () => {
    for (const ext of ['.flac', '.wav', '.mp3', '.m4a', '.m4b', '.m4r', '.aac', '.aiff', '.aif', '.caf']) {
      expect(routePlayback(ext)).toBe('element')
    }
  })

  it('routes exotic containers through the proxy', () => {
    for (const ext of ['.ogg', '.oga', '.opus', '.wma', '.ape', '.ac3', '.amr', '.mka', '.webm']) {
