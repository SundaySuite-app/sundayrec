...
describe('buildExportRequest', () => {
  ...
  it('passes a picked folder through untouched', () => {
    const params = buildExportRequest({ ...base, outputFolder: '/Users/x/Skrivebord' })
  ...
  it('never sends a mode field', () => {
    ...
    expect(Object.keys(buildExportRequest({ ...base, kind: 'video' }))).not.toContain('mode')
  })

  it('maps the audio format and bitrate onto the backend field names', () => {
    const params = buildExportRequest({ ...base, format: 'aac', bitrate: 192, bitDepth: 24 })
