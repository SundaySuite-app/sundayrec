...
describe('pairRecordings', () => {
  ...
  it('does NOT pair two recordings that merely share a start time', () => {
    ...
    const rows = [
      row({ path: '/rec/sal-a.mp4', date: '2026-05-31', startTime: '11:00', note: 'Video' }),
...
describe('historyTotals', () => {
  it('sums the real seconds instead of re-parsing the display label', () => {
    ...
    const rows = [
      row({ durationSec: 5400, duration: '1t 30m', date: '2026-05-31T11:00:00.000Z', timestamp: 3 }),
      row({ durationSec: 45, duration: '45 s', date: '2026-05-24T11:00:00.000Z', timestamp: 2 }),
      row({ durationSec: 600, duration: '10m', date: '2026-05-17T11:00:00.000Z', timestamp: 1 }),
    ]
  ...
  it('reports zeroes for an empty list', () => {
    expect(historyTotals([])).toEqual({ count: 0, totalSec: 0, lastDate: undefined })
  })
})
