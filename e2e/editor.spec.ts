...
// SMOKE-TEST §10b: the transcription panel has NO calm feature-gate hint. In a
// `--no-default-features` build `whisper_transcribe` rejects with
// `feature_disabled`, and that rejection must surface through the ordinary
// error dialog with its real reason — not vanish, and not masquerade as a
// model/download problem. (The model registry commands work in every build, so
// the modal itself opens normally either way.)
test.describe("transcription feature gate", () => {
  test("a feature_disabled transcribe surfaces its reason in the error dialog", async ({
    page,
  }) => {
    await openEditor(page, {
      whisper_list_models: [
        {
          ...
          label: "Tiny",
          quality: "best",
          description: "liten og rask",
          realtimeFactor: 10,
          sizeBytes: 75_000_000,
        },
      ],
      // Installed + intact, so startTranscription skips the download branch
      // and goes straight to the transcribe invoke.
      whisper_model_status: { installed: true, sizeOk: true },
      whisper_transcribe: fn(
        '() => { throw new Error("validation: feature_disabled: transkribering er ikke bygget inn i denne versjonen") }',
      ),
    });

    // The Transkribering panel lives on the content tab.
    await page.locator("#editor-tab-content").click();
    await page.locator("#btn-transcribe").click();
    await expect(page.locator("#transcribe-model-list")).toBeVisible();
    await page.locator("#btn-transcribe-start").click();

    const dialog = page.locator(".ui-dialog");
    await expect(dialog).toBeVisible();
    await expect(dialog).toContainText("Transkribering feilet");
    await expect(dialog).toContainText("feature_disabled");
  });
});
